	...
	$.fn.leafletLocationPicker = function (opts, onChangeLocation) {
		...
		var baseClassName = 'leaflet-locpicker',
			baseLayers = {
				...
				mapTailer: 'https://api.maptiler.com/maps/streets-v2/{z}/{x}/{y}.png?key=wS9EzK6H2X0P1pl3sagx',
		...
		function buildMap(self) {
			...
			if (opts.readOnly) self.$map.addClass('read-only');

			//adds either as global div or specified container
			//if added to specified container add some style class
			if (opts.mapContainer && $(opts.mapContainer)) self.$map.appendTo(opts.mapContainer).addClass('map-select');
			...
			self.map = L.map(self.divMap, opts.map)
				...
				.on(opts.event, function (e) {
					if (!opts.readOnly) self.setLocation(e.latlng);
				});
			...
			if (opts.alwaysOpen !== true) {
				...
				xmap.onAdd = function (map) {
					...
					L.DomEvent.on(btn, 'click', L.DomEvent.stop, self).on(btn, 'click', self.closeMap, self);
					return btn_holder;
				};
				xmap.addTo(self.map);
			}
		...
		function buildMarker(loc) {
			return L.marker(parseLocation(loc) || L.latLng(0, 0), {
				icon: L.divIcon({
					...
					html:
						'<div' +
						('30px' !== opts.cursorSize
							? 'style="width: ' + opts.cursorSize + '; height: ' + opts.cursorSize + ';"'
							: '') +
						'>' +
