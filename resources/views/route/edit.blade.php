...
@section('content')
	<x-card>
		<form id="submit-form" method="post" action="{{ route('route.update', $route->id) }}"
			class="repeater tw-mt-6 tw-space-y-6">
			@csrf
			@method('PUT')
			...
			<div class="form-group">
				...
				<textarea name="description" id="description"
					class="form-control">{{old('description', $route->description)}}</textarea>
			</div>
			...
			<div class="form-group">
				...
				<select name="direction" class="custom-select">
					<option value="clockwise" @if (old('direction', $route->direction) === 'clockwise') selected @endif>
						Clockwise</option>
					<option value="anti_clockwise" @if (old('direction', $route->direction) === 'anti_clockwise') selected @endif>
