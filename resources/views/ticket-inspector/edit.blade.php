...
@section('content')
	<x-card>
		<form id="submit-form" method="post" action="{{ route('ticket-inspector.update', $ticket_inspector->id) }}"
			...
			<div class="form-group">
				...
				<x-text-input id="name" name="name" type="text" class="tw-mt-1 tw-block tw-w-full"
					value="{{ old('name', default: $ticket_inspector->name) }}" />
			</div>

			<div class="form-group">
				<x-input-label for="email" value="Email" />
				<x-text-input id="email" name="email" type="email" class="tw-mt-1 tw-block tw-w-full"
					value="{{ old('email', default: $ticket_inspector->email) }}" />
			</div>
